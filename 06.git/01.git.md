...
|模式 (Mode)|HEAD (分支指针)|索引 (Index/Staging Area)|工作目录 (Working Directory)|简述用途|
|:--|:--|:--|:--|:--|
|`--mixed` (默认)|移动|重置 到新 HEAD|不变|撤销提交并取消暂存。保留文件更改，但需要重新暂存才能提交|
|`--soft`|移动|不变|不变|仅撤销提交。将所有更改保留在索引和工作目录中，等待重新提交|
|`--hard`| 移动|  重置 到新 HEAD| 重置 到新 HEAD|  完全撤销。丢弃提交、取消暂存，并销毁工作目录中的所有本地更改|

- 要保留更改并立即重新提交 使用 `--soft`
- 要保留更改但想重新组织提交内容 使用 `--mixed`
- 要彻底丢弃所有更改 使用 `--hard`
...
# 设置upstream
git remote add upstream git@github.com:zhaopan/base-pub.git

# git remote -v
origin   git@github.com:zhaopan/pub.git (fetch)
origin   git@github.com:zhaopan/pub.git (push)
upstream git@github.com:zhaopan/base-pub.git (fetch)
upstream git@github.com:zhaopan/base-pub.git (push)
...
- --allow-unrelated-histories: 这个标志告诉 Git，即使这两个分支的历史看起来是完全不相关的，也请继续执行合并操作。

# merge-dir

# 读取远程目录到索引
git read-tree upstream/master:dir/dir
# 强制检出所有文件到当前目录

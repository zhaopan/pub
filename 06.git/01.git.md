...
- [Git](#git)
  ...
  - [log show grep](#log-show-grep)
  - [remote](#remote)
  - [reset checkout](#reset-checkout)
  - [sparse-checkout](#sparse-checkout)
  - [git 迁移](#git-迁移)
  - [fork后如何与原仓库同步](#fork后如何与原仓库同步)
  - [rebase](#rebase)
  - [常规命令](#常规命令)
  - [清理仓库](#清理仓库)
  - [Reference](#reference)
...
请参考考 [INTERACTIVE MODE](https://git-scm.com/docs/git-rebase#_interactive_mode)
...
#p，pick=使用提交（不能修改提交内容，即文件内容）
#r，reword=使用提交，但编辑提交消息,说明已经很清楚了，就是可以编辑 commit message。 ( 不能修改 commit 内容，也就是 files 内容 )
#e，edit=使用commit，但停止修改 简单地说，reword 只可以修改提交消息，而编辑不单可以修改提交消息，也可以修改文件内容。
#s，squash=使用提交，但合并到上一次提交中,简单说，你如果想要将多个 commit 合并成一个
#f，fixup=类似“squash”，但放弃此提交的日志消息,其实这个和 squash 很像，通常如果我们要忽略一个 commit message 但保留 commit 的内容
#x，exec=使用shell运行命令（行的其余部分）
#d，drop=删除提交,这个就简单多了，移除这个 commit ( 包含 commit 内容 )，
```
...
# 编辑上次提交
git commit --amend -m "reword commit msg"
...
# 参考链接  # https://www.codenong.com/2919878/
